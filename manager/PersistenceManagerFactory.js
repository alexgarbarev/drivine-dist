...
let PersistenceManagerFactory = (() => {
    let PersistenceManagerFactory = class PersistenceManagerFactory {
        ...
        buildOrResolve(database = 'default', type = 'DELEGATING') {
        ...
        register(name) {
            ...
            this.managers.set(name, {
                transactional: new TransactionalPersistenceManager_1.TransactionalPersistenceManager(this.contextHolder, name),
                nonTransactional: new NonTransactionalPersistenceManager_1.NonTransactionalPersistenceManager(connectionProvider),
                delegating: new DelegatingPersistenceManager_1.DelegatingPersistenceManager(name, this.contextHolder, this)
            });
        }
    };
