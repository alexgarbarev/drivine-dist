...
const fs = require('fs');
let DrivineModuleBuilder = (() => {
    class DrivineModuleBuilder {
        constructor(options) {
            this.options = options;
            this.logger = new common_1.Logger(DrivineModuleBuilder.name);
        }
        get providers() {
            if (!this._providers) {
                this._providers = [
        ...
        persistenceManagers() {
            return DrivineInjectionDecorators_1.persistenceManagerInjections.map((database) => {
                return {
        ...
        cypherStatementProviders() {
            return DrivineInjectionDecorators_1.cypherInjections.map((path) => {
                ...
                return {
                    provide: token,
                    useFactory: () => Statement_1.cypherStatement(this.fileContents(path))
                };
            });
        }
        sqlStatementProviders() {
            return DrivineInjectionDecorators_1.sqlInjections.map((path) => {
                ...
                return {
                    provide: token,
                    useFactory: () => Statement_1.sqlStatement(this.fileContents(path))
                };
            });
        }
        fileContents(path) {
