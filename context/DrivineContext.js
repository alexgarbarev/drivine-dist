...
class DrivineContext {
    ...
    withTransaction(options) {
        this.transactionOptions = Transactional_1.optionsWithDefaults(options);
        return this;
    }
    async run(fn) {
        if (this.contextHolder.drivineContext) {
            return this.transactionOptions ? Transactional_1.runInTransaction(fn, this.transactionOptions) : fn();
        }
        else {
